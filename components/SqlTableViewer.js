...
const search_url = 'http://localhost:3001/api/spacs/sql'
...
const FieldTitle = ({ title , hoverColor, color, onFieldClick}) => {
    ...
    return (
        <div
            className={`flex h-full mx-1 hover:"bg-blue-200" ${color} cursor-default`}
...
const FieldsViewer = ({ fields_list, selected_field, selectField }) => {
    ...
    return (
        <div className="flex w-full h-auto rounded-md border-b-2 overflow-y-scroll">
...
const DocunentViewer = ({ document }) => {
    ...
    return (
        <div className="relative flex flex-1 flex-col justify-center items-center  bg-gray-200">
            <div className="absolute flex justify-center top-0 left-0 w-full h-20 my-2">

                <div className="flex h-full w-2/5 justify-center items-center text-4xl font-black rounded-lg shadow-softdark">Document</div>
            </div>
            <div className="w-full h-20"/>
            <div className="h-100 w-10/12 m-10 overflow-y-scroll bg-slate-300 border-2 border-gray-600 rounded-lg">
                <FieldsView
                title
                hitSource={document}
                tlwind_customized
...
const DocumentsFieldView = ({fieldOnFocuse, field_values, selectValue, selected_value}) => {
    ...
    return (
        <div className="flex flex-1 flex-col justify-start items-center">
            <div className="flex justify-start items-start w-1/2 text-4xl m-5">{fieldOnFocuse}</div>
...
const DocumentsView = ({ values_documents_list, values_list, fieldOnFocuse}) => {
    ...
    return (
        <div className="flex flex-row justify-center items-start w-full border-2 p-5">
...
const SqlTableViewer = ({ table_name, searchQuery}) => {
    ...
    useEffect(() => {
        if (!onload  && data.query != searchQuery)
        {
            ...
            fetch(query_url,{
                ...
                console.log("***********SQQQLLLLL table Viewerrrrr fetched data : ", query_res)
                setData({
    ...
    return (
        <div className="w-full flex flex-col justify-start items-center px-16 bg-white">
        {!onload && <div className="flex justify-center items-center w-full h-50 text-4xl">{table_name}</div>}
        {!onload && <FieldsViewer
